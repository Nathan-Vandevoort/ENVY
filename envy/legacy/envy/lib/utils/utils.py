...
def get_server_ip(logger: logging.Logger = None) -> str:  # returns the ip address of the server and null if failed
    ...
    server_file_path = os.path.join(envy.__file__, 'Connections', 'server.txt')
...
def get_clients_from_file(logger: logging.Logger = None) -> dict:
    ...
    """
    logger = logger or DummyLogger()
    clients_file_path = os.path.join(envy.__file__, 'Connections', 'clients.json')
    clients = None
...
def validate_classifier(classifier: str, logger: logging.Logger = None) -> bool:
    ...
    if (
        re.search(
            '^lab[0-9*-]+\s?[0-9*-]*$|^vr\s?[0-9*-]+$|^lab[0-9]{1,2}[0-9a-z-]+[0-9]{2}$|^vr[0-9]{1,2}[0-9a-z-]+[' '0-9]{2}$',
            classifier,
            re.IGNORECASE,
        )
        is not None
    ):  # check if classifier starts with lab or vr followed by acceptable characters and ending with a number or *
        return True
    return False
...
def validate_computer_against_classifier(classifier: str, hostname: str, logger: logging.Logger = None) -> bool:
    ...
    for token in tokens:
        ...
        if re.search('^[0-9]{0,2}(-)[0-9]{0,2}$', token, re.IGNORECASE):  # checks and processes if computer number is a range
            greaterThanTenTest = [(char, len(char) == 2) for char in token.split('-')]
            newToken = [f"0[{char}-9]$|" if flag != True else f"[a-z0-9\-]*1[0-{char[-1:]}]$" for char, flag in greaterThanTenTest]
        ...
        if re.search('^[^A-Za-z0-9]', token, re.IGNORECASE):  # checks if token is a special character (meant to catch wild cards)
            computer = token
...
def get_applicable_clients(classifier: str, clients: typing.Iterable, logger: logging.Logger = None) -> list:
    """
...
def make_job_directories():
    ...
    :return: Void
    """
    os.makedirs(os.path.join(envy.__file__, 'Jobs', 'Jobs'))
