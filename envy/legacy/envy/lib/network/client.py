...
from envy.lib.network import exceptions as network_exceptions
...
class Client:
    ...
    def __init__(
        ...
        purpose: MessagePurpose = MessagePurpose.CLIENT,
    ):
    ...
    async def health_check_server(self) -> bool:
        ...
        status, exception, data = await self.connect(purpose=MessagePurpose.HEALTH_CHECK, use_logger=False)
        if not status:
            return False
        return True

    async def connect(self, purpose=MessagePurpose.CLIENT, timeout=2.5, use_logger: bool = True) -> tuple:
        ...
        headers = {'Passkey': self.hash, 'Name': self.hostname}

        try:
            if purpose == MessagePurpose.HEALTH_CHECK:
            ...
            if purpose == MessagePurpose.CLIENT:
                websocket = await websockets.connect(uri, extra_headers=headers, timeout=timeout)
                if websocket.open:
                    self.websocket = websocket
                    return True, None, 'Connected Successfully'

            if purpose == MessagePurpose.CONSOLE:
                websocket = await websockets.connect(uri, extra_headers=headers, timeout=timeout)
                if websocket.open:
                    self.websocket = websocket
                    return True, None, 'Connected Successfully'
